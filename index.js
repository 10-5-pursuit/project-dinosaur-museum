...
const carnivorousTrace = createScatterPlot(
  carnivorousData,
  "Carnivorous",
  "red"
);
const herbivorousTrace = createScatterPlot(
  herbivorousData,
  "Herbivorous",
  "green"
);
...
function createDinosaurTable(data) {
  const tableData = data.map((dino, index) => ({
    dinoCount: index + 1, // Adjust the index to start at 1
    Dinosaur: dino.name,
...
console.log(
  "Included another column as a Dino Counter [i+1].  Used console.table to generate this. First discovered it during the library/fetching books-authors-genre lesson. Glad to try it out on this project."
);

// Room 2 Stretch Goals:

// To make Rizel's Jurassic Cafe
const cafeRoom = {
...
// added these two rooms to the Museum
const rooms = [cafeRoom, giftShopRoom];
...
const tipPercentage = 15; // Replace this with the desired tip percentage
...
console.log(
  "Prehistoric delights, just for you! Thanks for choosing Rizel's Jurassic Cafe. 🦕🍰"
);
...
console.log(
  ...
  ticketDetailsForFellow.fullPriceInDollars
);
console.log(
  "Discounts Applied:",
  ticketDetailsForFellow.discountsApplied.join(", ")
);
console.log("Net Price to Pay:", ticketDetailsForFellow.netPriceToPay);
console.log("-----------------------------");
console.log(
  "Made a scatter graph of Dino Diet vs Dino Length using plotly. Click on url!"
);
