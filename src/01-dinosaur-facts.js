...
function getLongestDinosaur(dinosaurs) {
  ...
  let resultDino = {};
  let longestDino = {lengthInMeters: -Infinity};
  ...
  resultDino[name] = lengthInMeters*3.281;
...
function getDinosaurDescription(dinosaurs, id) {
  for (const dino of dinosaurs) 
    if (dino.dinosaurId == id){
      ...
      return `${name} (${pronunciation})\n${info} It lived in the ${period} period, over ${Math.min(...mya)} million years ago.`
    }
...
function getDinosaursAliveMya(dinosaurs, mya, key) {
  let aliveDinos = [];
  for (const dino of dinosaurs)
    if ((dino.mya.length == 1 && (dino.mya[0] == mya || dino.mya[0] == mya+1)) || (dino.mya[1] <= mya && dino.mya[0] >= mya)) {
      if (key && key in dino)
